...
get_interpreter( bash ) -> "bash";
get_interpreter( r ) -> "R --vanilla --slave".
...
%% get_assignment/4
%
get_assignment( bash, Name, false, Value ) ->
  flatten( [Name, "=", list_quoted( Value ), "\n"] );
  
get_assignment( r, Name, false, Value ) ->
  flatten( [Name, "=", list_quoted( Value ), "\n"] );
...
get_dismissal( r, Name, false ) ->
  "cat(\""++?CFMSG++Name++?COLON++"[\","++Name++",\"]\\n\")\n";
  
get_dismissal( r, Name, true ) ->
  "cat(\""++?CFMSG++Name++?COLON++"[\",paste(\"\\\"\","++Name++",\"\\\"\",collapse=\",\",sep=\"\"),\"]\\n\",sep=\"\")\n".
